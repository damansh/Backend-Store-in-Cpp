...
private:
   ...
   bool insertHelper(TreeNode* root, DBentry* newEntry);
   DBentry* findHelper(TreeNode* root, string name);
   bool removeHelper(TreeNode* prev, TreeNode* root, string name);
   void clearHelper(TreeNode* root);
   int countInventoryHelper(TreeNode* root) const;
    void printAllHelper(TreeNode* root, ostream& out) const;
